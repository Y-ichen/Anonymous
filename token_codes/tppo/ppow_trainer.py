...
class PPOWTrainer(BaseTrainer):
    ...
    def get_suggested_lengths(self, masks):
        ...
        for k , mask in enumerate(masks):
            ...
            if len(segment_lengths) > 0:
                ...
                median_length = (sorted_lengths[mid] + sorted_lengths[~mid]) / 2 if len(sorted_lengths) % 2 == 0 else sorted_lengths[mid]
            else:
                median_length = 0   
            
            # 4. 用中位数乘以段数作为suggested length -> 改成了直接乘以10
            # suggested_length = int(median_length * len(segment_lengths))
            suggested_length = int(median_length * 10)
    ...
    def get_length_weight_penalty(self, alpha, suggested_lengths, scores):
        ...
        for k, score in enumerate(scores):
            ...
            for i in range(start_index, score_tensor.numel()):
                len += 1
                # 3. 计算这个位置的lwp, 控制指数范围在[-10, 6]之间
                # lwp_len = 1.0 / (1.0 + math.exp(alpha * (len - suggested_lengths[k]) - 6))
    ...
    def step(
        ...
        # get suggest length
        suggested_lengths = self.get_suggested_lengths(masks)
        
        # calculate length weight penalty
        lwps, scores_penaled = self.get_length_weight_penalty(alpha, suggested_lengths, scores)
        
        # resign value of penaled scores to scores
        # scores = torch.tensor(scores_penaled)
        ...
        with torch.no_grad():
            ...
            if full_kl_penalty:
                ...
                #:) TODO: rematch scores with words (longformer tokens) and all_tokens (mistral tokens) 
                rewards, non_score_reward, kls = self.compute_rewards(scores, all_logprobs, ref_logprobs, masks, all_tokens, words)
    ...
    def batched_forward_pass(
        ...
        for i in range(math.ceil(bs / fbs)):
            ...
            for k in range(len(masks)):
                mask = masks[k]
                token = all_tokens[k]
                # Find the indices of ',' in token, where the mask is 1
                comma_indices = [i for i, char in enumerate(token) if char == ',' and mask[i] == 1]
    ...
    def assign_token_rewards(self, token_list, word_list, score_list):
        ...
        while token_index < len(token_list)-1:
            # assert word_index < len(word_list) # TODO: fix it
            if word_index >= len(word_list):
                print('word_list:', word_list)
                print('token_list:', token_list)
                token_score.append(torch.tensor(0., device=score_list[0].device))
                token_index += 1
                continue
            flag_matched = False
            i_matched = 0
            wid_matched = -1
            for i in range(1, min(6, len(token_list)-token_index)):
                sublist = token_list[token_index:token_index+i]
            ...
            if flag_matched:
                token_score.extend([score_list[wid_matched]]*i_matched)
        ...
        if len(token_score) != len(token_list):
            token_score = token_score + [torch.tensor(0., device=score_list[0].device)] * (len(token_list) - len(token_score))
        
        # assert len(token_score) == len(token_list)
        return token_score
    ...
    def rematch_scores(self, scores, words, all_tokens):
        ...
        # 使用列表推导式获取每个子列表中字符串"answer"的下标
        answer_indices_words = [(sublist.index("answer")+2) if "answer" in sublist else -1 for sublist in words]
        ...
        # 使用列表推导式查找前后是'an'和'swer'两个字符串的索引
        answer_indices_tokens = [
            (i + 1 + 3)
            for sublist in all_tokens
            for i in range(len(sublist) - 1)
            if sublist[i] == 'an' and sublist[i + 1] == 'swer'
        ]
        ...
        for i in range(len(words)):
            if answer_indices_words[i] != -1:
                answers_words.append(words[i][answer_indices_words[i]:])
                answers_scores.append(scores[i][answer_indices_words[i]:])
            else:
                answers_words.append([])
        ...
        # merge by looking for ġ
        answers_words_reformed = []
        answers_scores_reformed = []
        for i, word_list in enumerate(answers_words):
            new_word_list, new_word_indices = self.merge_words(word_list)
            word_scores = self.get_word_score_by_indices(answers_scores[i], new_word_indices)
            
            if len(new_word_list) != len(word_scores):
                word_scores = word_scores + [torch.tensor(0., device=word_scores[0].device)] * (len(new_word_list) - len(word_scores))
            
            answers_words_reformed.append(new_word_list)
            answers_scores_reformed.append(word_scores)
            
            # assert len(new_word_list) == len(word_scores)
    
        token_scores = []        
        for i, token_list in enumerate(answers_tokens):
            word_list = answers_words_reformed[i]
            score_list = answers_scores_reformed[i]
            
            token_non_score = [torch.tensor(0., device=score_list[0].device)] * answer_indices_tokens[i]
            token_score = self.assign_token_rewards(token_list, word_list, score_list)
    ...
    def compute_advantages(
        ...
        for t in reversed(range(gen_len)):
            ...
            delta = rewards[:, t] + self.config.gamma * nextvalues - values[:, t] # reward
    ...
    def record_step_stats(self, kl_coef: float, **data):
        ...
        # TODO: select the scores where the mask is 1
        masked_scores = []
    ...
    def log_stats(
        ...
        if not isinstance(rewards, torch.Tensor):
            rewards = [reward.mean() for reward in rewards] # TODO: add mask for rewards
